    ...
    void Traffic_light::image_cb(const smartcar_msgs::ImageObjectsConstPtr &obj)
    {
        ...
        for (auto it = Lightobjs.begin(); it != Lightobjs.end(); ++it)
        {
            ...
            if (label.compare("traffic_light") == 0)
            {
                ...
                if(lightcount>10)lightcount=10;
                light_result.reset(new smartcar_msgs::TrafficLightResult);
                light_result->distance = distance;
                float sizeScale=0.3;
                cv::Rect l_roi=cv::Rect(rect_roi.x,rect_roi.y+rect_roi.height/2,
                                        rect_roi.width,rect_roi.height*2/3);
                cv::rectangle(workimage, l_roi, cv::Scalar(0, 0, 0), 2);
                cv::Mat lightimage=workimage(l_roi);
                this->setMask(lightimage);
